...
const StudentListingPage = () => {
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get(API_PATHS.ADMIN.ALL_STUDENT_USER, {
        params: {
          page: currentPage - 1,
          size: pageSize,
      ...
      setTotalStudents(response.data.totalElements || response.data.length);
      setLoading(false);
    } catch (err) {
  ...
  const handleActionMenu = (studentId) => {
    if (actionMenuOpen === studentId) {
      setActionMenuOpen(null);
    } else {
      setActionMenuOpen(studentId);
    }
  };
  ...
  const handleDeleteStudent = async (studentId) => {
    if (window.confirm('Are you sure you want to delete this student?')) {
      try {
        await axiosInstance.delete(`${API_PATHS.ADMIN.STUDENT_USER}/${studentId}`);
  ...
  const handleExportData = () => {
    const csvContent = [
      ['ID', 'Username', 'Email', 'Created Date', 'Updated Date', 'Status'],
  ...
  const endIndex = Math.min(startIndex + pageSize - 1, totalStudents);
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {students.length === 0 ? (
                    ...
                    students.map((student) => (
                      <tr key={student.id} className={`hover:bg-gray-50 ${student.is_deleted ? 'bg-red-50' : ''}`}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                          <div className="relative">
                            <button
                              onClick={() => handleActionMenu(student.id)}
                                ...
                                {!student.is_deleted ? (
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      console.log('Restore student:', student.id);
                                      setActionMenuOpen(null);
                                    }}
